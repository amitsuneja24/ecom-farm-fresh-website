...
class Checkout extends Component {
    ...
    async componentDidMount() {
        ...
        let grandTotal = cart.reduce((sum, i) => (sum += i.qty * i.netPrice), 0)
        let discount = cart.reduce((sum, i) => (sum += i.discount * i.qty), 0)
                                        ...
                                        cartItems.map((row, index) => (
                                            <div className="card-body pt-0 pr-0 pl-0 pb-0" key={index}>
                                                <div className="cart-list-product">
                                                    ...
                                                    <p className="offer-price mb-0">&#x20B9;{row.netPrice + '*' + row.qty} <i className="mdi mdi-tag-outline" /> <span className="regular-price">&#x20B9;{row.price}</span></p>
                                                </div>
                                            </div>
                                        ))
                                    }
